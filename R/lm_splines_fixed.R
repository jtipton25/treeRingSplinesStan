...
#'
#' @return A stan object that is the result of fitting the spline regression model
#' @export
#'
lm_splines_fixed <- function(y, X, X_bs, n_plot, n_tree, plot_by_tree_idx, tree_idx, X_pred, X_bs_pred, tree_idx_pred, ...) {
  ...
  out <- rstan::sampling(stanmodels$splines_fixed, data = standata, ...)
  return(out)

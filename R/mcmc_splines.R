...
#' @param Xbs A n times df*q matrix of b-spline expanded covariate values
#' @param df The b-spline degree of freedom parameter
...
#' @export
#'
mcmc_splines <- function(y, X, Xbs, plot_idx, tree_idx, params, config = NULL, verbose = FALSE, n_chain = 1) {
  ...
  ## do we sample the spline parameters?
  sample_beta_bs <- TRUE
  if (!is.null(config)) {
    if (!is.null(config[['sample_beta_bs']])) {
      sample_beta_bs <- config[['sample_beta_bs']]
      if (!is.logical(sample_beta_bs) | is.na(sample_beta_bs))
        stop('If specified, sample_beta_bs must be TRUE or FALSE')
    }
  }
  ...
  # spline penalty prior
  q <- ncol(Xbs) / df
  bs_idx <- rep(1:q, each = df)

  mu_beta_bs   <- rep(0, ncol(Xbs))
  tau2_beta_bs <- runif(q, 1, 10)
  Q    <- spam(c(make_Q_1d(df, phi = 1)), df, df)
  Qbs  <- do.call(bdiag.spam, sapply(1:q, function(i) tau2_beta_bs[i] * Q))
  Abs  <- tXXbs / sigma2 + Qbs
  Rstruct <- chol(Abs)
  bbs <- 1 / sigma2 * tXbs %*% (y - X_intercept %*% beta_intercept - X %*% beta) + Qbs %*% mu_beta_bs

  beta_bs <- drop(rmvnorm.canonical(1, bbs, Abs, Rstruct = Rstruct))
  ...
  tau2_beta_bs_save   <- matrix(0, n_save, q)
  beta_bs_save        <- matrix(0, n_save, ncol(Xbs))
  ...
  for (k in 1:(n_adapt + n_mcmc)) {
    ...
    if (sample_beta_intercept) {
      ...
      b              <- 1 / sigma2 * tX_int %*% (y - X %*% beta - Xbs %*% beta_bs) + Q_beta_intercept %*% mu_beta_intercept
    ...
    if (sample_beta) {
      ...
      b      <- 1 / sigma2 * tX %*% (y - X_intercept %*% beta_intercept - Xbs %*% beta_bs) + Q_beta %*% mu_beta
      A      <- (A + t(A)) / 2 # guarantee a symmetric matrix
      beta   <- rmvn_arma(A, b)
    }
    ...
    if (sample_sigma2) {
      ...
      devs      <- y - X_intercept %*% beta_intercept - X %*% beta - Xbs %*% beta_bs
      SS        <- sum(devs^2)
    ...
    SS        <- sum(devs)^2
    s2_beta_p <- 1 / rgamma(1, n_intercept[2] / 2 + 10, SS / 2 + 0.1)

    ## sample_s_beta_t
    devs      <- beta_intercept[intercept_idx == "tree"]
    SS        <- sum(devs)^2
    s2_beta_t <- 1 / rgamma(1, n_intercept[3] / 2 + 10, SS / 2 + 0.1)
    ...
    for (j in 1:q) {
      devs            <-  beta_bs[bs_idx == j]
      SS              <- as.numeric(devs %*% (Q %*% devs))
      # prior at high values of tau2 (small variance for strong shrinkage)
      tau2_beta_bs[j] <- rgamma(1, 20 + df / 2, 0.5 + SS / 2)
    }
    ...
    if (k >= n_adapt) {
      if (k %% n_thin == 0) {
        ...
        beta_bs_save[save_idx, ]        <- beta_bs
        tau2_beta_bs_save[save_idx, ]   <- tau2_beta_bs
        sigma2_save[save_idx]           <- sigma2
        s2_beta_p_save[save_idx]        <- s2_beta_p
        s2_beta_t_save[save_idx]        <- s2_beta_t
        lp_save[save_idx, ]             <- dnorm(y, X_intercept %*% beta_intercept + X %*% beta + Xbs %*% beta_bs, sqrt(sigma2), log = TRUE)
      }
    }
  }
  ...
  out <- list(
    ...
    beta_bs        = beta_bs_save,
    sigma2         = sigma2_save,
    s2_beta_p      = s2_beta_p_save,
    s2_beta_t      = s2_beta_t_save,
    tau2_beta_bs   = tau2_beta_bs_save,
    lp__           = lp_save
  )

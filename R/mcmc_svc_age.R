...
mcmc_svc_age <- function(
  ...
  sample_beta_age <- TRUE
  if (!is.null(config)) {
    if (!is.null(config[['sample_beta_age']])) {
      sample_beta_age <- config[['sample_beta_age']]
      if (!is.logical(sample_beta_age) | is.na(sample_beta_age))
        stop('If specified, sample_beta_age must be TRUE or FALSE')
    }
  }
  ...
  sample_tau2_age <- TRUE
  if (!is.null(config)) {
    if (!is.null(config[['sample_tau2_age']])) {
      sample_tau2_age <- config[['sample_tau2_age']]
      if (!is.logical(sample_tau2_age) | is.na(sample_tau2_age))
        stop('If specified, sample_tau2_age must be TRUE or FALSE')
    }
  }
  ...
  Q0 <- bdiag.spam(25, tau20 * make_Q(df, 0.99))
  Q_age <- tau2_age * make_Q_1d(df, 0.99)
  Q <- list(length = p)
  for (j in 1:p) {
    Q[[j]] <- bdiag.spam(25, tau2[j] * make_Q(df, 0.99))
  }
  beta0 <- drop(rmvnorm.prec(1, rep(0, df^2+1), Q0))
  Rstruct0 <- chol(1 / sigma2 * tWW + Q0)
  beta_age <- drop(rmvnorm.prec(1, rep(0, df), Q_age))
  Rstruct_age <- chol(1 / sigma2 * tWW_age + Q_age)
  ...
  ##
  Q0 <- bdiag.spam(25, tau20 * make_Q(df, 1))
  Q_age <- tau2_age * make_Q_1d(df, 1)
  Q <- list(length = p)
  for (j in 1:p) {
    Q[[j]] <- bdiag.spam(25, tau2[j] * make_Q(df, 1))
  }
  ...
  if (!is.null(inits[['beta0']])) {
    if(!is_numeric_vector(inits[['beta0']], df^2 + 1))
      stop("initial value for beta0 must be a numeric vector of length df^2 + 1")
  ...
  ## initial values for beta_age
  if (!is.null(inits[['beta_age']])) {
    if(!is_numeric_vector(inits[['beta_age']], df))
      stop("initial value for beta_age must be a numeric vector of length df")
    if (all(!is.na(inits[['beta_age']]))) {
      beta <- inits[['beta_age']]
    }
  }
  Wbeta_age  <- W_age %*% beta_age
  ...
  if (!is.null(inits[['beta']])) {
    if(!is_numeric_matrix(inits[['beta']], df^2 + 1, p))
      stop("initial value for beta must be a numeric vector of length df^2 + 1")
  ...
  if (!is.null(inits[['tau20']])) {
    if(!is_positive_numeric(inits[['tau20']], 1))
      stop("initial value for tau20 must be a positive numeric value")
  ...
  ## initial values for tau2_age
  if (!is.null(inits[['tau2_age']])) {
    if(!is_positive_numeric(inits[['tau20']], 1))
      stop("initial value for tau2_age must be a positive numeric value")
    if (all(!is.na(inits[['tau2_age']]))) {
      tau2_age <- inits[['tau2_age']]
    }
  }
  # update Q0 with the initial value
  Q_age <- tau2_age * make_Q_1d(df, 1)
  ...
  beta_age_save <- matrix(0, n_save, df)
  beta_save    <- array(0, dim = c(n_save, df^2 + 1, p))
  tau20_save   <- rep(0, n_save)
  tau2_age_save <- matrix(0, n_save, 1)
  ...
  for (k in 1:(params$n_adapt + params$n_mcmc)) {
    ...
    if (sample_sigma2) {
      ...
      devs   <- y - Wbeta0 - Wbeta_age - apply(XWbeta, 1, sum)
      SS     <- sum(devs^2)
    ...
    if (sample_beta0) {
      ...
      A     <- 1 / sigma2 * tWW + Q0
      b     <- 1 / sigma2 * tW %*% (y - Wbeta_age - apply(XWbeta, 1, sum))
    ...
    ##
    ## sample beta_age -- double check these values
    ##

    if (sample_beta_age) {
      if (verbose)
        message("sample beta_age")

      # A      <- as.matrix(1 / sigma2 * tWW + Q0)
      A     <- 1 / sigma2 * tWW_age + Q_age
      b     <- 1 / sigma2 * tW_age %*% (y - Wbeta0 - apply(XWbeta, 1, sum))
      ## guarantee a symmetric matrix
      A     <- (A + t(A)) / 2
      # beta0  <- drop(rmvn_arma(A, b))
      beta_age <- drop(rmvnorm.canonical(1, b, A, Rstruct = Rstruct_age))

      ## update Wbeta_age
      Wbeta_age <- drop(W_age %*% beta_age)
    }
    ...
    if (sample_beta) {
      ...
      for (j in 1:p) {
        ...
        b <- drop(1 / sigma2 * tXW[[j]] %*% (y - Wbeta0 - Wbeta_age - apply(XWbeta[, -j, drop = FALSE], 1, sum)))
        ## guarantee a symmetric matrix
        A         <- (A + t(A)) / 2
    ...
    if (sample_tau20) {
      ...
      devs  <- beta0[-1]
      SS    <- as.numeric(devs %*% (Q0[-1, -1] %*% devs)) / tau20
    ...
    if (sample_tau2_age) {
      if (verbose)
        message("sample tau20")
      devs  <- beta_age
      SS    <- as.numeric(devs %*% (Q_age %*% devs)) / tau2_age
      tau2_age <- rgamma(1, alpha_tau2 + df / 2, beta_tau2 + SS / 2)
    }
    ...
    if (sample_tau2) {
      ...
      for (j in 1:p) {
        devs    <- beta[-1, j]
        SS      <- as.numeric(devs %*% (Q[[j]][-1, -1] %*% devs)) / tau2[j]
    ...
    if (k >= params$n_adapt) {
      if (k %% params$n_thin == 0) {
        ...
        beta_age_save[save_idx, ]  <- beta_age
        beta_save[save_idx, , ] <- beta
        tau20_save[save_idx]    <- tau20
        tau2_age_save[save_idx] <- tau2_age
  ...
  out <- list(
    beta0  = beta0_save,
    beta_age = beta_age_save,
    beta   = beta_save,
    tau20  = tau20_save,
    tau2_age = tau2_age_save,
    tau2   = tau2_save,
    sigma2 = sigma2_save,
    W      = W,
    W_age  = W_age,

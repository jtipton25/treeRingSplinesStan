// Generated by rstantools.  Do not edit by hand.

/*
    treeRingSplines is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    treeRingSplines is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with treeRingSplines.  If not, see <http://www.gnu.org/licenses/>.
*/
#ifndef MODELS_HPP
#define MODELS_HPP
#define STAN__SERVICES__COMMAND_HPP
#include <rstan/rstaninc.hpp>
// Code generated by Stan version 2.21.0
#include <stan/model/model_header.hpp>
namespace model_splines_interaction_sparse_namespace {
using std::istream;
using std::string;
using std::stringstream;
using std::vector;
using stan::io::dump;
using stan::math::lgamma;
using stan::model::prob_grad;
using namespace stan::math;
static int current_statement_begin__;
stan::io::program_reader prog_reader__() {
    stan::io::program_reader reader;
    reader.add_event(0, 0, "start", "model_splines_interaction_sparse");
    reader.add_event(225, 223, "end", "model_splines_interaction_sparse");
    return reader;
}
template <bool propto, typename T0__, typename T1__, typename T3__, typename T4__>
typename boost::math::tools::promote_args<T0__, T1__, T3__, T4__>::type
sparse_iar_lpdf(const Eigen::Matrix<T0__, Eigen::Dynamic, 1>& phi,
                    const T1__& tau,
                    const std::vector<std::vector<int> >& W_sparse,
                    const Eigen::Matrix<T3__, Eigen::Dynamic, 1>& D_sparse,
                    const Eigen::Matrix<T4__, Eigen::Dynamic, 1>& lambda,
                    const int& df_int,
                    const int& W_n, std::ostream* pstream__) {
    typedef typename boost::math::tools::promote_args<T0__, T1__, T3__, T4__>::type local_scalar_t__;
    typedef local_scalar_t__ fun_return_scalar_t__;
    const static bool propto__ = true;
    (void) propto__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // suppress unused var warning
    int current_statement_begin__ = -1;
    try {
        {
        current_statement_begin__ = 19;
        validate_non_negative_index("phit_D", "df_int", df_int);
        Eigen::Matrix<local_scalar_t__, 1, Eigen::Dynamic> phit_D(df_int);
        stan::math::initialize(phit_D, DUMMY_VAR__);
        stan::math::fill(phit_D, DUMMY_VAR__);
        current_statement_begin__ = 20;
        validate_non_negative_index("phit_W", "df_int", df_int);
        Eigen::Matrix<local_scalar_t__, 1, Eigen::Dynamic> phit_W(df_int);
        stan::math::initialize(phit_W, DUMMY_VAR__);
        stan::math::fill(phit_W, DUMMY_VAR__);
        current_statement_begin__ = 21;
        validate_non_negative_index("ldet_terms", "df_int", df_int);
        Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> ldet_terms(df_int);
        stan::math::initialize(ldet_terms, DUMMY_VAR__);
        stan::math::fill(ldet_terms, DUMMY_VAR__);
        current_statement_begin__ = 23;
        stan::math::assign(phit_D, transpose(elt_multiply(phi, D_sparse)));
        current_statement_begin__ = 24;
        stan::math::assign(phit_W, rep_row_vector(0, df_int));
        current_statement_begin__ = 25;
        for (int i = 1; i <= W_n; ++i) {
            current_statement_begin__ = 26;
            stan::model::assign(phit_W, 
                        stan::model::cons_list(stan::model::index_uni(get_base1(get_base1(W_sparse, i, "W_sparse", 1), 1, "W_sparse", 2)), stan::model::nil_index_list()), 
                        (get_base1(phit_W, get_base1(get_base1(W_sparse, i, "W_sparse", 1), 1, "W_sparse", 2), "phit_W", 1) + get_base1(phi, get_base1(get_base1(W_sparse, i, "W_sparse", 1), 2, "W_sparse", 2), "phi", 1)), 
                        "assigning variable phit_W");
            current_statement_begin__ = 27;
            stan::model::assign(phit_W, 
                        stan::model::cons_list(stan::model::index_uni(get_base1(get_base1(W_sparse, i, "W_sparse", 1), 2, "W_sparse", 2)), stan::model::nil_index_list()), 
                        (get_base1(phit_W, get_base1(get_base1(W_sparse, i, "W_sparse", 1), 2, "W_sparse", 2), "phit_W", 1) + get_base1(phi, get_base1(get_base1(W_sparse, i, "W_sparse", 1), 1, "W_sparse", 2), "phi", 1)), 
                        "assigning variable phit_W");
        }
        current_statement_begin__ = 30;
        return stan::math::promote_scalar<fun_return_scalar_t__>((0.5 * (((df_int - 1) * stan::math::log(tau)) - (tau * (multiply(phit_D, phi) - multiply(phit_W, phi))))));
        }
    } catch (const std::exception& e) {
        stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
        // Next line prevents compiler griping about no return
        throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
    }
}
template <typename T0__, typename T1__, typename T3__, typename T4__>
typename boost::math::tools::promote_args<T0__, T1__, T3__, T4__>::type
sparse_iar_lpdf(const Eigen::Matrix<T0__, Eigen::Dynamic, 1>& phi,
                    const T1__& tau,
                    const std::vector<std::vector<int> >& W_sparse,
                    const Eigen::Matrix<T3__, Eigen::Dynamic, 1>& D_sparse,
                    const Eigen::Matrix<T4__, Eigen::Dynamic, 1>& lambda,
                    const int& df_int,
                    const int& W_n, std::ostream* pstream__) {
    return sparse_iar_lpdf<false>(phi,tau,W_sparse,D_sparse,lambda,df_int,W_n, pstream__);
}
struct sparse_iar_lpdf_functor__ {
    template <bool propto, typename T0__, typename T1__, typename T3__, typename T4__>
        typename boost::math::tools::promote_args<T0__, T1__, T3__, T4__>::type
    operator()(const Eigen::Matrix<T0__, Eigen::Dynamic, 1>& phi,
                    const T1__& tau,
                    const std::vector<std::vector<int> >& W_sparse,
                    const Eigen::Matrix<T3__, Eigen::Dynamic, 1>& D_sparse,
                    const Eigen::Matrix<T4__, Eigen::Dynamic, 1>& lambda,
                    const int& df_int,
                    const int& W_n, std::ostream* pstream__) const {
        return sparse_iar_lpdf(phi, tau, W_sparse, D_sparse, lambda, df_int, W_n, pstream__);
    }
};
#include <stan_meta_header.hpp>
class model_splines_interaction_sparse
  : public stan::model::model_base_crtp<model_splines_interaction_sparse> {
private:
        int n;
        int K;
        int p;
        int df;
        matrix_d X;
        std::vector<matrix_d> X_bs;
        int q;
        int df_int;
        std::vector<matrix_d> X_bs_int;
        matrix_d W;
        int W_n;
        vector_d y;
        int n_plot;
        int n_tree;
        std::vector<int> plot_by_tree_idx;
        std::vector<int> tree_idx;
        int n_pred;
        matrix_d X_pred;
        std::vector<matrix_d> X_bs_pred;
        std::vector<matrix_d> X_bs_int_pred;
        std::vector<int> tree_idx_pred;
        std::vector<std::vector<int> > W_sparse;
        vector_d D_sparse;
        vector_d lambda;
        vector_d zeros;
public:
    model_splines_interaction_sparse(stan::io::var_context& context__,
        std::ostream* pstream__ = 0)
        : model_base_crtp(0) {
        ctor_body(context__, 0, pstream__);
    }
    model_splines_interaction_sparse(stan::io::var_context& context__,
        unsigned int random_seed__,
        std::ostream* pstream__ = 0)
        : model_base_crtp(0) {
        ctor_body(context__, random_seed__, pstream__);
    }
    void ctor_body(stan::io::var_context& context__,
                   unsigned int random_seed__,
                   std::ostream* pstream__) {
        typedef double local_scalar_t__;
        boost::ecuyer1988 base_rng__ =
          stan::services::util::create_rng(random_seed__, 0);
        (void) base_rng__;  // suppress unused var warning
        current_statement_begin__ = -1;
        static const char* function__ = "model_splines_interaction_sparse_namespace::model_splines_interaction_sparse";
        (void) function__;  // dummy to suppress unused var warning
        size_t pos__;
        (void) pos__;  // dummy to suppress unused var warning
        std::vector<int> vals_i__;
        std::vector<double> vals_r__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // suppress unused var warning
        try {
            // initialize data block variables from context__
            current_statement_begin__ = 37;
            context__.validate_dims("data initialization", "n", "int", context__.to_vec());
            n = int(0);
            vals_i__ = context__.vals_i("n");
            pos__ = 0;
            n = vals_i__[pos__++];
            check_greater_or_equal(function__, "n", n, 0);
            current_statement_begin__ = 38;
            context__.validate_dims("data initialization", "K", "int", context__.to_vec());
            K = int(0);
            vals_i__ = context__.vals_i("K");
            pos__ = 0;
            K = vals_i__[pos__++];
            check_greater_or_equal(function__, "K", K, 0);
            current_statement_begin__ = 39;
            context__.validate_dims("data initialization", "p", "int", context__.to_vec());
            p = int(0);
            vals_i__ = context__.vals_i("p");
            pos__ = 0;
            p = vals_i__[pos__++];
            check_greater_or_equal(function__, "p", p, 0);
            current_statement_begin__ = 40;
            context__.validate_dims("data initialization", "df", "int", context__.to_vec());
            df = int(0);
            vals_i__ = context__.vals_i("df");
            pos__ = 0;
            df = vals_i__[pos__++];
            check_greater_or_equal(function__, "df", df, 0);
            current_statement_begin__ = 41;
            validate_non_negative_index("X", "n", n);
            validate_non_negative_index("X", "K", K);
            context__.validate_dims("data initialization", "X", "matrix_d", context__.to_vec(n,K));
            X = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(n, K);
            vals_r__ = context__.vals_r("X");
            pos__ = 0;
            size_t X_j_2_max__ = K;
            size_t X_j_1_max__ = n;
            for (size_t j_2__ = 0; j_2__ < X_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < X_j_1_max__; ++j_1__) {
                    X(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 42;
            validate_non_negative_index("X_bs", "n", n);
            validate_non_negative_index("X_bs", "df", df);
            validate_non_negative_index("X_bs", "p", p);
            context__.validate_dims("data initialization", "X_bs", "matrix_d", context__.to_vec(p,n,df));
            X_bs = std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> >(p, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(n, df));
            vals_r__ = context__.vals_r("X_bs");
            pos__ = 0;
            size_t X_bs_j_2_max__ = df;
            size_t X_bs_j_1_max__ = n;
            size_t X_bs_k_0_max__ = p;
            for (size_t j_2__ = 0; j_2__ < X_bs_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < X_bs_j_1_max__; ++j_1__) {
                    for (size_t k_0__ = 0; k_0__ < X_bs_k_0_max__; ++k_0__) {
                        X_bs[k_0__](j_1__, j_2__) = vals_r__[pos__++];
                    }
                }
            }
            current_statement_begin__ = 43;
            context__.validate_dims("data initialization", "q", "int", context__.to_vec());
            q = int(0);
            vals_i__ = context__.vals_i("q");
            pos__ = 0;
            q = vals_i__[pos__++];
            check_greater_or_equal(function__, "q", q, 0);
            current_statement_begin__ = 44;
            context__.validate_dims("data initialization", "df_int", "int", context__.to_vec());
            df_int = int(0);
            vals_i__ = context__.vals_i("df_int");
            pos__ = 0;
            df_int = vals_i__[pos__++];
            check_greater_or_equal(function__, "df_int", df_int, 0);
            current_statement_begin__ = 45;
            validate_non_negative_index("X_bs_int", "n", n);
            validate_non_negative_index("X_bs_int", "df_int", df_int);
            validate_non_negative_index("X_bs_int", "q", q);
            context__.validate_dims("data initialization", "X_bs_int", "matrix_d", context__.to_vec(q,n,df_int));
            X_bs_int = std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> >(q, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(n, df_int));
            vals_r__ = context__.vals_r("X_bs_int");
            pos__ = 0;
            size_t X_bs_int_j_2_max__ = df_int;
            size_t X_bs_int_j_1_max__ = n;
            size_t X_bs_int_k_0_max__ = q;
            for (size_t j_2__ = 0; j_2__ < X_bs_int_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < X_bs_int_j_1_max__; ++j_1__) {
                    for (size_t k_0__ = 0; k_0__ < X_bs_int_k_0_max__; ++k_0__) {
                        X_bs_int[k_0__](j_1__, j_2__) = vals_r__[pos__++];
                    }
                }
            }
            current_statement_begin__ = 46;
            validate_non_negative_index("W", "df_int", df_int);
            validate_non_negative_index("W", "df_int", df_int);
            context__.validate_dims("data initialization", "W", "matrix_d", context__.to_vec(df_int,df_int));
            W = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(df_int, df_int);
            vals_r__ = context__.vals_r("W");
            pos__ = 0;
            size_t W_j_2_max__ = df_int;
            size_t W_j_1_max__ = df_int;
            for (size_t j_2__ = 0; j_2__ < W_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < W_j_1_max__; ++j_1__) {
                    W(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 47;
            context__.validate_dims("data initialization", "W_n", "int", context__.to_vec());
            W_n = int(0);
            vals_i__ = context__.vals_i("W_n");
            pos__ = 0;
            W_n = vals_i__[pos__++];
            current_statement_begin__ = 48;
            validate_non_negative_index("y", "n", n);
            context__.validate_dims("data initialization", "y", "vector_d", context__.to_vec(n));
            y = Eigen::Matrix<double, Eigen::Dynamic, 1>(n);
            vals_r__ = context__.vals_r("y");
            pos__ = 0;
            size_t y_j_1_max__ = n;
            for (size_t j_1__ = 0; j_1__ < y_j_1_max__; ++j_1__) {
                y(j_1__) = vals_r__[pos__++];
            }
            current_statement_begin__ = 50;
            context__.validate_dims("data initialization", "n_plot", "int", context__.to_vec());
            n_plot = int(0);
            vals_i__ = context__.vals_i("n_plot");
            pos__ = 0;
            n_plot = vals_i__[pos__++];
            check_greater_or_equal(function__, "n_plot", n_plot, 0);
            current_statement_begin__ = 51;
            context__.validate_dims("data initialization", "n_tree", "int", context__.to_vec());
            n_tree = int(0);
            vals_i__ = context__.vals_i("n_tree");
            pos__ = 0;
            n_tree = vals_i__[pos__++];
            check_greater_or_equal(function__, "n_tree", n_tree, 0);
            current_statement_begin__ = 52;
            validate_non_negative_index("plot_by_tree_idx", "n_tree", n_tree);
            context__.validate_dims("data initialization", "plot_by_tree_idx", "int", context__.to_vec(n_tree));
            plot_by_tree_idx = std::vector<int>(n_tree, int(0));
            vals_i__ = context__.vals_i("plot_by_tree_idx");
            pos__ = 0;
            size_t plot_by_tree_idx_k_0_max__ = n_tree;
            for (size_t k_0__ = 0; k_0__ < plot_by_tree_idx_k_0_max__; ++k_0__) {
                plot_by_tree_idx[k_0__] = vals_i__[pos__++];
            }
            size_t plot_by_tree_idx_i_0_max__ = n_tree;
            for (size_t i_0__ = 0; i_0__ < plot_by_tree_idx_i_0_max__; ++i_0__) {
                check_greater_or_equal(function__, "plot_by_tree_idx[i_0__]", plot_by_tree_idx[i_0__], 1);
            }
            current_statement_begin__ = 53;
            validate_non_negative_index("tree_idx", "n", n);
            context__.validate_dims("data initialization", "tree_idx", "int", context__.to_vec(n));
            tree_idx = std::vector<int>(n, int(0));
            vals_i__ = context__.vals_i("tree_idx");
            pos__ = 0;
            size_t tree_idx_k_0_max__ = n;
            for (size_t k_0__ = 0; k_0__ < tree_idx_k_0_max__; ++k_0__) {
                tree_idx[k_0__] = vals_i__[pos__++];
            }
            size_t tree_idx_i_0_max__ = n;
            for (size_t i_0__ = 0; i_0__ < tree_idx_i_0_max__; ++i_0__) {
                check_greater_or_equal(function__, "tree_idx[i_0__]", tree_idx[i_0__], 1);
            }
            current_statement_begin__ = 55;
            context__.validate_dims("data initialization", "n_pred", "int", context__.to_vec());
            n_pred = int(0);
            vals_i__ = context__.vals_i("n_pred");
            pos__ = 0;
            n_pred = vals_i__[pos__++];
            check_greater_or_equal(function__, "n_pred", n_pred, 0);
            current_statement_begin__ = 56;
            validate_non_negative_index("X_pred", "n_pred", n_pred);
            validate_non_negative_index("X_pred", "K", K);
            context__.validate_dims("data initialization", "X_pred", "matrix_d", context__.to_vec(n_pred,K));
            X_pred = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(n_pred, K);
            vals_r__ = context__.vals_r("X_pred");
            pos__ = 0;
            size_t X_pred_j_2_max__ = K;
            size_t X_pred_j_1_max__ = n_pred;
            for (size_t j_2__ = 0; j_2__ < X_pred_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < X_pred_j_1_max__; ++j_1__) {
                    X_pred(j_1__, j_2__) = vals_r__[pos__++];
                }
            }
            current_statement_begin__ = 57;
            validate_non_negative_index("X_bs_pred", "n_pred", n_pred);
            validate_non_negative_index("X_bs_pred", "df", df);
            validate_non_negative_index("X_bs_pred", "p", p);
            context__.validate_dims("data initialization", "X_bs_pred", "matrix_d", context__.to_vec(p,n_pred,df));
            X_bs_pred = std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> >(p, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(n_pred, df));
            vals_r__ = context__.vals_r("X_bs_pred");
            pos__ = 0;
            size_t X_bs_pred_j_2_max__ = df;
            size_t X_bs_pred_j_1_max__ = n_pred;
            size_t X_bs_pred_k_0_max__ = p;
            for (size_t j_2__ = 0; j_2__ < X_bs_pred_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < X_bs_pred_j_1_max__; ++j_1__) {
                    for (size_t k_0__ = 0; k_0__ < X_bs_pred_k_0_max__; ++k_0__) {
                        X_bs_pred[k_0__](j_1__, j_2__) = vals_r__[pos__++];
                    }
                }
            }
            current_statement_begin__ = 58;
            validate_non_negative_index("X_bs_int_pred", "n_pred", n_pred);
            validate_non_negative_index("X_bs_int_pred", "df_int", df_int);
            validate_non_negative_index("X_bs_int_pred", "q", q);
            context__.validate_dims("data initialization", "X_bs_int_pred", "matrix_d", context__.to_vec(q,n_pred,df_int));
            X_bs_int_pred = std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> >(q, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>(n_pred, df_int));
            vals_r__ = context__.vals_r("X_bs_int_pred");
            pos__ = 0;
            size_t X_bs_int_pred_j_2_max__ = df_int;
            size_t X_bs_int_pred_j_1_max__ = n_pred;
            size_t X_bs_int_pred_k_0_max__ = q;
            for (size_t j_2__ = 0; j_2__ < X_bs_int_pred_j_2_max__; ++j_2__) {
                for (size_t j_1__ = 0; j_1__ < X_bs_int_pred_j_1_max__; ++j_1__) {
                    for (size_t k_0__ = 0; k_0__ < X_bs_int_pred_k_0_max__; ++k_0__) {
                        X_bs_int_pred[k_0__](j_1__, j_2__) = vals_r__[pos__++];
                    }
                }
            }
            current_statement_begin__ = 59;
            validate_non_negative_index("tree_idx_pred", "n_pred", n_pred);
            context__.validate_dims("data initialization", "tree_idx_pred", "int", context__.to_vec(n_pred));
            tree_idx_pred = std::vector<int>(n_pred, int(0));
            vals_i__ = context__.vals_i("tree_idx_pred");
            pos__ = 0;
            size_t tree_idx_pred_k_0_max__ = n_pred;
            for (size_t k_0__ = 0; k_0__ < tree_idx_pred_k_0_max__; ++k_0__) {
                tree_idx_pred[k_0__] = vals_i__[pos__++];
            }
            size_t tree_idx_pred_i_0_max__ = n_pred;
            for (size_t i_0__ = 0; i_0__ < tree_idx_pred_i_0_max__; ++i_0__) {
                check_greater_or_equal(function__, "tree_idx_pred[i_0__]", tree_idx_pred[i_0__], 1);
            }
            // initialize transformed data variables
            current_statement_begin__ = 63;
            validate_non_negative_index("W_sparse", "W_n", W_n);
            validate_non_negative_index("W_sparse", "2", 2);
            W_sparse = std::vector<std::vector<int> >(W_n, std::vector<int>(2, int(0)));
            stan::math::fill(W_sparse, std::numeric_limits<int>::min());
            current_statement_begin__ = 64;
            validate_non_negative_index("D_sparse", "df_int", df_int);
            D_sparse = Eigen::Matrix<double, Eigen::Dynamic, 1>(df_int);
            stan::math::fill(D_sparse, DUMMY_VAR__);
            current_statement_begin__ = 65;
            validate_non_negative_index("lambda", "df_int", df_int);
            lambda = Eigen::Matrix<double, Eigen::Dynamic, 1>(df_int);
            stan::math::fill(lambda, DUMMY_VAR__);
            current_statement_begin__ = 66;
            validate_non_negative_index("zeros", "df_int", df_int);
            zeros = Eigen::Matrix<double, Eigen::Dynamic, 1>(df_int);
            stan::math::fill(zeros, DUMMY_VAR__);
            // execute transformed data statements
            current_statement_begin__ = 68;
            for (int j = 1; j <= df_int; ++j) {
                current_statement_begin__ = 69;
                stan::model::assign(zeros, 
                            stan::model::cons_list(stan::model::index_uni(j), stan::model::nil_index_list()), 
                            0.0, 
                            "assigning variable zeros");
            }
            {
            current_statement_begin__ = 73;
            int counter(0);
            (void) counter;  // dummy to suppress unused var warning
            stan::math::fill(counter, std::numeric_limits<int>::min());
            current_statement_begin__ = 74;
            stan::math::assign(counter, 1);
            current_statement_begin__ = 76;
            for (int i = 1; i <= (df_int - 1); ++i) {
                current_statement_begin__ = 77;
                for (int j = (i + 1); j <= df_int; ++j) {
                    current_statement_begin__ = 78;
                    if (as_bool(logical_eq(get_base1(W, i, j, "W", 1), 1))) {
                        current_statement_begin__ = 79;
                        stan::model::assign(W_sparse, 
                                    stan::model::cons_list(stan::model::index_uni(counter), stan::model::cons_list(stan::model::index_uni(1), stan::model::nil_index_list())), 
                                    i, 
                                    "assigning variable W_sparse");
                        current_statement_begin__ = 80;
                        stan::model::assign(W_sparse, 
                                    stan::model::cons_list(stan::model::index_uni(counter), stan::model::cons_list(stan::model::index_uni(2), stan::model::nil_index_list())), 
                                    j, 
                                    "assigning variable W_sparse");
                        current_statement_begin__ = 81;
                        stan::math::assign(counter, (counter + 1));
                    }
                }
            }
            }
            current_statement_begin__ = 86;
            for (int i = 1; i <= df_int; ++i) {
                current_statement_begin__ = 86;
                stan::model::assign(D_sparse, 
                            stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                            sum(get_base1(W, i, "W", 1)), 
                            "assigning variable D_sparse");
            }
            {
            current_statement_begin__ = 88;
            validate_non_negative_index("invsqrtD", "df_int", df_int);
            Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> invsqrtD(df_int);
            stan::math::initialize(invsqrtD, DUMMY_VAR__);
            stan::math::fill(invsqrtD, DUMMY_VAR__);
            current_statement_begin__ = 89;
            for (int i = 1; i <= df_int; ++i) {
                current_statement_begin__ = 90;
                stan::model::assign(invsqrtD, 
                            stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                            (1 / stan::math::sqrt(get_base1(D_sparse, i, "D_sparse", 1))), 
                            "assigning variable invsqrtD");
            }
            current_statement_begin__ = 92;
            stan::math::assign(lambda, eigenvalues_sym(quad_form(W, diag_matrix(invsqrtD))));
            }
            // validate transformed data
            // validate, set parameter ranges
            num_params_r__ = 0U;
            param_ranges_i__.clear();
            current_statement_begin__ = 98;
            num_params_r__ += 1;
            current_statement_begin__ = 99;
            validate_non_negative_index("beta0_p_tilde", "n_plot", n_plot);
            num_params_r__ += (1 * n_plot);
            current_statement_begin__ = 100;
            num_params_r__ += 1;
            current_statement_begin__ = 101;
            validate_non_negative_index("beta0_t_tilde", "n_tree", n_tree);
            num_params_r__ += (1 * n_tree);
            current_statement_begin__ = 102;
            num_params_r__ += 1;
            current_statement_begin__ = 105;
            validate_non_negative_index("beta", "K", K);
            num_params_r__ += K;
            current_statement_begin__ = 108;
            validate_non_negative_index("beta_bs", "df", df);
            validate_non_negative_index("beta_bs", "p", p);
            num_params_r__ += (df * p);
            current_statement_begin__ = 109;
            validate_non_negative_index("sigma_beta", "p", p);
            num_params_r__ += (1 * p);
            current_statement_begin__ = 112;
            validate_non_negative_index("beta_int", "df_int", df_int);
            validate_non_negative_index("beta_int", "q", q);
            num_params_r__ += (df_int * q);
            current_statement_begin__ = 113;
            validate_non_negative_index("sigma_beta_int", "q", q);
            num_params_r__ += (1 * q);
            current_statement_begin__ = 116;
            num_params_r__ += 1;
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
            throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
        }
    }
    ~model_splines_interaction_sparse() { }
    void transform_inits(const stan::io::var_context& context__,
                         std::vector<int>& params_i__,
                         std::vector<double>& params_r__,
                         std::ostream* pstream__) const {
        typedef double local_scalar_t__;
        stan::io::writer<double> writer__(params_r__, params_i__);
        size_t pos__;
        (void) pos__; // dummy call to supress warning
        std::vector<double> vals_r__;
        std::vector<int> vals_i__;
        current_statement_begin__ = 98;
        if (!(context__.contains_r("mu_beta0")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable mu_beta0 missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("mu_beta0");
        pos__ = 0U;
        context__.validate_dims("parameter initialization", "mu_beta0", "double", context__.to_vec());
        double mu_beta0(0);
        mu_beta0 = vals_r__[pos__++];
        try {
            writer__.scalar_unconstrain(mu_beta0);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable mu_beta0: ") + e.what()), current_statement_begin__, prog_reader__());
        }
        current_statement_begin__ = 99;
        if (!(context__.contains_r("beta0_p_tilde")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable beta0_p_tilde missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("beta0_p_tilde");
        pos__ = 0U;
        validate_non_negative_index("beta0_p_tilde", "n_plot", n_plot);
        context__.validate_dims("parameter initialization", "beta0_p_tilde", "double", context__.to_vec(n_plot));
        std::vector<double> beta0_p_tilde(n_plot, double(0));
        size_t beta0_p_tilde_k_0_max__ = n_plot;
        for (size_t k_0__ = 0; k_0__ < beta0_p_tilde_k_0_max__; ++k_0__) {
            beta0_p_tilde[k_0__] = vals_r__[pos__++];
        }
        size_t beta0_p_tilde_i_0_max__ = n_plot;
        for (size_t i_0__ = 0; i_0__ < beta0_p_tilde_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_unconstrain(beta0_p_tilde[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable beta0_p_tilde: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 100;
        if (!(context__.contains_r("s_beta0_p")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable s_beta0_p missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("s_beta0_p");
        pos__ = 0U;
        context__.validate_dims("parameter initialization", "s_beta0_p", "double", context__.to_vec());
        double s_beta0_p(0);
        s_beta0_p = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0, s_beta0_p);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable s_beta0_p: ") + e.what()), current_statement_begin__, prog_reader__());
        }
        current_statement_begin__ = 101;
        if (!(context__.contains_r("beta0_t_tilde")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable beta0_t_tilde missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("beta0_t_tilde");
        pos__ = 0U;
        validate_non_negative_index("beta0_t_tilde", "n_tree", n_tree);
        context__.validate_dims("parameter initialization", "beta0_t_tilde", "double", context__.to_vec(n_tree));
        std::vector<double> beta0_t_tilde(n_tree, double(0));
        size_t beta0_t_tilde_k_0_max__ = n_tree;
        for (size_t k_0__ = 0; k_0__ < beta0_t_tilde_k_0_max__; ++k_0__) {
            beta0_t_tilde[k_0__] = vals_r__[pos__++];
        }
        size_t beta0_t_tilde_i_0_max__ = n_tree;
        for (size_t i_0__ = 0; i_0__ < beta0_t_tilde_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_unconstrain(beta0_t_tilde[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable beta0_t_tilde: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 102;
        if (!(context__.contains_r("s_beta0_t")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable s_beta0_t missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("s_beta0_t");
        pos__ = 0U;
        context__.validate_dims("parameter initialization", "s_beta0_t", "double", context__.to_vec());
        double s_beta0_t(0);
        s_beta0_t = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0, s_beta0_t);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable s_beta0_t: ") + e.what()), current_statement_begin__, prog_reader__());
        }
        current_statement_begin__ = 105;
        if (!(context__.contains_r("beta")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable beta missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("beta");
        pos__ = 0U;
        validate_non_negative_index("beta", "K", K);
        context__.validate_dims("parameter initialization", "beta", "vector_d", context__.to_vec(K));
        Eigen::Matrix<double, Eigen::Dynamic, 1> beta(K);
        size_t beta_j_1_max__ = K;
        for (size_t j_1__ = 0; j_1__ < beta_j_1_max__; ++j_1__) {
            beta(j_1__) = vals_r__[pos__++];
        }
        try {
            writer__.vector_unconstrain(beta);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable beta: ") + e.what()), current_statement_begin__, prog_reader__());
        }
        current_statement_begin__ = 108;
        if (!(context__.contains_r("beta_bs")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable beta_bs missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("beta_bs");
        pos__ = 0U;
        validate_non_negative_index("beta_bs", "df", df);
        validate_non_negative_index("beta_bs", "p", p);
        context__.validate_dims("parameter initialization", "beta_bs", "vector_d", context__.to_vec(p,df));
        std::vector<Eigen::Matrix<double, Eigen::Dynamic, 1> > beta_bs(p, Eigen::Matrix<double, Eigen::Dynamic, 1>(df));
        size_t beta_bs_j_1_max__ = df;
        size_t beta_bs_k_0_max__ = p;
        for (size_t j_1__ = 0; j_1__ < beta_bs_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_bs_k_0_max__; ++k_0__) {
                beta_bs[k_0__](j_1__) = vals_r__[pos__++];
            }
        }
        size_t beta_bs_i_0_max__ = p;
        for (size_t i_0__ = 0; i_0__ < beta_bs_i_0_max__; ++i_0__) {
            try {
                writer__.vector_unconstrain(beta_bs[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable beta_bs: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 109;
        if (!(context__.contains_r("sigma_beta")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable sigma_beta missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("sigma_beta");
        pos__ = 0U;
        validate_non_negative_index("sigma_beta", "p", p);
        context__.validate_dims("parameter initialization", "sigma_beta", "double", context__.to_vec(p));
        std::vector<double> sigma_beta(p, double(0));
        size_t sigma_beta_k_0_max__ = p;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_k_0_max__; ++k_0__) {
            sigma_beta[k_0__] = vals_r__[pos__++];
        }
        size_t sigma_beta_i_0_max__ = p;
        for (size_t i_0__ = 0; i_0__ < sigma_beta_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_lb_unconstrain(0, sigma_beta[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable sigma_beta: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 112;
        if (!(context__.contains_r("beta_int")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable beta_int missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("beta_int");
        pos__ = 0U;
        validate_non_negative_index("beta_int", "df_int", df_int);
        validate_non_negative_index("beta_int", "q", q);
        context__.validate_dims("parameter initialization", "beta_int", "vector_d", context__.to_vec(q,df_int));
        std::vector<Eigen::Matrix<double, Eigen::Dynamic, 1> > beta_int(q, Eigen::Matrix<double, Eigen::Dynamic, 1>(df_int));
        size_t beta_int_j_1_max__ = df_int;
        size_t beta_int_k_0_max__ = q;
        for (size_t j_1__ = 0; j_1__ < beta_int_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_int_k_0_max__; ++k_0__) {
                beta_int[k_0__](j_1__) = vals_r__[pos__++];
            }
        }
        size_t beta_int_i_0_max__ = q;
        for (size_t i_0__ = 0; i_0__ < beta_int_i_0_max__; ++i_0__) {
            try {
                writer__.vector_unconstrain(beta_int[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable beta_int: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 113;
        if (!(context__.contains_r("sigma_beta_int")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable sigma_beta_int missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("sigma_beta_int");
        pos__ = 0U;
        validate_non_negative_index("sigma_beta_int", "q", q);
        context__.validate_dims("parameter initialization", "sigma_beta_int", "double", context__.to_vec(q));
        std::vector<double> sigma_beta_int(q, double(0));
        size_t sigma_beta_int_k_0_max__ = q;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_int_k_0_max__; ++k_0__) {
            sigma_beta_int[k_0__] = vals_r__[pos__++];
        }
        size_t sigma_beta_int_i_0_max__ = q;
        for (size_t i_0__ = 0; i_0__ < sigma_beta_int_i_0_max__; ++i_0__) {
            try {
                writer__.scalar_lb_unconstrain(0, sigma_beta_int[i_0__]);
            } catch (const std::exception& e) {
                stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable sigma_beta_int: ") + e.what()), current_statement_begin__, prog_reader__());
            }
        }
        current_statement_begin__ = 116;
        if (!(context__.contains_r("sigma_y")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable sigma_y missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("sigma_y");
        pos__ = 0U;
        context__.validate_dims("parameter initialization", "sigma_y", "double", context__.to_vec());
        double sigma_y(0);
        sigma_y = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0, sigma_y);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable sigma_y: ") + e.what()), current_statement_begin__, prog_reader__());
        }
        params_r__ = writer__.data_r();
        params_i__ = writer__.data_i();
    }
    void transform_inits(const stan::io::var_context& context,
                         Eigen::Matrix<double, Eigen::Dynamic, 1>& params_r,
                         std::ostream* pstream__) const {
      std::vector<double> params_r_vec;
      std::vector<int> params_i_vec;
      transform_inits(context, params_i_vec, params_r_vec, pstream__);
      params_r.resize(params_r_vec.size());
      for (int i = 0; i < params_r.size(); ++i)
        params_r(i) = params_r_vec[i];
    }
    template <bool propto__, bool jacobian__, typename T__>
    T__ log_prob(std::vector<T__>& params_r__,
                 std::vector<int>& params_i__,
                 std::ostream* pstream__ = 0) const {
        typedef T__ local_scalar_t__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // dummy to suppress unused var warning
        T__ lp__(0.0);
        stan::math::accumulator<T__> lp_accum__;
        try {
            stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
            // model parameters
            current_statement_begin__ = 98;
            local_scalar_t__ mu_beta0;
            (void) mu_beta0;  // dummy to suppress unused var warning
            if (jacobian__)
                mu_beta0 = in__.scalar_constrain(lp__);
            else
                mu_beta0 = in__.scalar_constrain();
            current_statement_begin__ = 99;
            std::vector<local_scalar_t__> beta0_p_tilde;
            size_t beta0_p_tilde_d_0_max__ = n_plot;
            beta0_p_tilde.reserve(beta0_p_tilde_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < beta0_p_tilde_d_0_max__; ++d_0__) {
                if (jacobian__)
                    beta0_p_tilde.push_back(in__.scalar_constrain(lp__));
                else
                    beta0_p_tilde.push_back(in__.scalar_constrain());
            }
            current_statement_begin__ = 100;
            local_scalar_t__ s_beta0_p;
            (void) s_beta0_p;  // dummy to suppress unused var warning
            if (jacobian__)
                s_beta0_p = in__.scalar_lb_constrain(0, lp__);
            else
                s_beta0_p = in__.scalar_lb_constrain(0);
            current_statement_begin__ = 101;
            std::vector<local_scalar_t__> beta0_t_tilde;
            size_t beta0_t_tilde_d_0_max__ = n_tree;
            beta0_t_tilde.reserve(beta0_t_tilde_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < beta0_t_tilde_d_0_max__; ++d_0__) {
                if (jacobian__)
                    beta0_t_tilde.push_back(in__.scalar_constrain(lp__));
                else
                    beta0_t_tilde.push_back(in__.scalar_constrain());
            }
            current_statement_begin__ = 102;
            local_scalar_t__ s_beta0_t;
            (void) s_beta0_t;  // dummy to suppress unused var warning
            if (jacobian__)
                s_beta0_t = in__.scalar_lb_constrain(0, lp__);
            else
                s_beta0_t = in__.scalar_lb_constrain(0);
            current_statement_begin__ = 105;
            Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> beta;
            (void) beta;  // dummy to suppress unused var warning
            if (jacobian__)
                beta = in__.vector_constrain(K, lp__);
            else
                beta = in__.vector_constrain(K);
            current_statement_begin__ = 108;
            std::vector<Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> > beta_bs;
            size_t beta_bs_d_0_max__ = p;
            beta_bs.reserve(beta_bs_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < beta_bs_d_0_max__; ++d_0__) {
                if (jacobian__)
                    beta_bs.push_back(in__.vector_constrain(df, lp__));
                else
                    beta_bs.push_back(in__.vector_constrain(df));
            }
            current_statement_begin__ = 109;
            std::vector<local_scalar_t__> sigma_beta;
            size_t sigma_beta_d_0_max__ = p;
            sigma_beta.reserve(sigma_beta_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < sigma_beta_d_0_max__; ++d_0__) {
                if (jacobian__)
                    sigma_beta.push_back(in__.scalar_lb_constrain(0, lp__));
                else
                    sigma_beta.push_back(in__.scalar_lb_constrain(0));
            }
            current_statement_begin__ = 112;
            std::vector<Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> > beta_int;
            size_t beta_int_d_0_max__ = q;
            beta_int.reserve(beta_int_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < beta_int_d_0_max__; ++d_0__) {
                if (jacobian__)
                    beta_int.push_back(in__.vector_constrain(df_int, lp__));
                else
                    beta_int.push_back(in__.vector_constrain(df_int));
            }
            current_statement_begin__ = 113;
            std::vector<local_scalar_t__> sigma_beta_int;
            size_t sigma_beta_int_d_0_max__ = q;
            sigma_beta_int.reserve(sigma_beta_int_d_0_max__);
            for (size_t d_0__ = 0; d_0__ < sigma_beta_int_d_0_max__; ++d_0__) {
                if (jacobian__)
                    sigma_beta_int.push_back(in__.scalar_lb_constrain(0, lp__));
                else
                    sigma_beta_int.push_back(in__.scalar_lb_constrain(0));
            }
            current_statement_begin__ = 116;
            local_scalar_t__ sigma_y;
            (void) sigma_y;  // dummy to suppress unused var warning
            if (jacobian__)
                sigma_y = in__.scalar_lb_constrain(0, lp__);
            else
                sigma_y = in__.scalar_lb_constrain(0);
            // transformed parameters
            current_statement_begin__ = 121;
            validate_non_negative_index("beta0_p", "n_plot", n_plot);
            std::vector<local_scalar_t__> beta0_p(n_plot, local_scalar_t__(0));
            stan::math::initialize(beta0_p, DUMMY_VAR__);
            stan::math::fill(beta0_p, DUMMY_VAR__);
            current_statement_begin__ = 122;
            validate_non_negative_index("beta0_t", "n_tree", n_tree);
            std::vector<local_scalar_t__> beta0_t(n_tree, local_scalar_t__(0));
            stan::math::initialize(beta0_t, DUMMY_VAR__);
            stan::math::fill(beta0_t, DUMMY_VAR__);
            current_statement_begin__ = 123;
            validate_non_negative_index("mu", "n", n);
            Eigen::Matrix<local_scalar_t__, Eigen::Dynamic, 1> mu(n);
            stan::math::initialize(mu, DUMMY_VAR__);
            stan::math::fill(mu, DUMMY_VAR__);
            current_statement_begin__ = 124;
            validate_non_negative_index("tau_beta_int", "q", q);
            std::vector<local_scalar_t__> tau_beta_int(q, local_scalar_t__(0));
            stan::math::initialize(tau_beta_int, DUMMY_VAR__);
            stan::math::fill(tau_beta_int, DUMMY_VAR__);
            // transformed parameters block statements
            current_statement_begin__ = 126;
            for (int j = 1; j <= n_plot; ++j) {
                current_statement_begin__ = 127;
                stan::model::assign(beta0_p, 
                            stan::model::cons_list(stan::model::index_uni(j), stan::model::nil_index_list()), 
                            (mu_beta0 + (s_beta0_p * get_base1(beta0_p_tilde, j, "beta0_p_tilde", 1))), 
                            "assigning variable beta0_p");
            }
            current_statement_begin__ = 130;
            for (int t = 1; t <= n_tree; ++t) {
                current_statement_begin__ = 131;
                stan::model::assign(beta0_t, 
                            stan::model::cons_list(stan::model::index_uni(t), stan::model::nil_index_list()), 
                            (get_base1(beta0_p, get_base1(plot_by_tree_idx, t, "plot_by_tree_idx", 1), "beta0_p", 1) + (s_beta0_t * get_base1(beta0_t_tilde, t, "beta0_t_tilde", 1))), 
                            "assigning variable beta0_t");
            }
            current_statement_begin__ = 134;
            for (int i = 1; i <= n; ++i) {
                {
                current_statement_begin__ = 136;
                local_scalar_t__ tmp(DUMMY_VAR__);
                (void) tmp;  // dummy to suppress unused var warning
                stan::math::initialize(tmp, DUMMY_VAR__);
                stan::math::fill(tmp, DUMMY_VAR__);
                stan::math::assign(tmp,0);
                current_statement_begin__ = 137;
                local_scalar_t__ tmp_int(DUMMY_VAR__);
                (void) tmp_int;  // dummy to suppress unused var warning
                stan::math::initialize(tmp_int, DUMMY_VAR__);
                stan::math::fill(tmp_int, DUMMY_VAR__);
                stan::math::assign(tmp_int,0);
                current_statement_begin__ = 140;
                for (int j = 1; j <= p; ++j) {
                    current_statement_begin__ = 141;
                    stan::math::assign(tmp, (tmp + multiply(get_base1(get_base1(X_bs, j, "X_bs", 1), i, "X_bs", 2), get_base1(beta_bs, j, "beta_bs", 1))));
                }
                current_statement_begin__ = 144;
                for (int j = 1; j <= q; ++j) {
                    current_statement_begin__ = 145;
                    stan::math::assign(tmp_int, (tmp_int + multiply(get_base1(get_base1(X_bs_int, j, "X_bs_int", 1), i, "X_bs_int", 2), get_base1(beta_int, j, "beta_int", 1))));
                }
                current_statement_begin__ = 147;
                stan::model::assign(mu, 
                            stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                            (((get_base1(beta0_t, get_base1(tree_idx, i, "tree_idx", 1), "beta0_t", 1) + multiply(get_base1(X, i, "X", 1), beta)) + tmp) + tmp_int), 
                            "assigning variable mu");
                }
            }
            current_statement_begin__ = 150;
            for (int j = 1; j <= q; ++j) {
                current_statement_begin__ = 151;
                stan::model::assign(tau_beta_int, 
                            stan::model::cons_list(stan::model::index_uni(j), stan::model::nil_index_list()), 
                            (1 / get_base1(sigma_beta_int, j, "sigma_beta_int", 1)), 
                            "assigning variable tau_beta_int");
            }
            // validate transformed parameters
            const char* function__ = "validate transformed params";
            (void) function__;  // dummy to suppress unused var warning
            current_statement_begin__ = 121;
            size_t beta0_p_k_0_max__ = n_plot;
            for (size_t k_0__ = 0; k_0__ < beta0_p_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(beta0_p[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: beta0_p" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable beta0_p: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 122;
            size_t beta0_t_k_0_max__ = n_tree;
            for (size_t k_0__ = 0; k_0__ < beta0_t_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(beta0_t[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: beta0_t" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable beta0_t: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 123;
            size_t mu_j_1_max__ = n;
            for (size_t j_1__ = 0; j_1__ < mu_j_1_max__; ++j_1__) {
                if (stan::math::is_uninitialized(mu(j_1__))) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: mu" << "(" << j_1__ << ")";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable mu: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            current_statement_begin__ = 124;
            size_t tau_beta_int_k_0_max__ = q;
            for (size_t k_0__ = 0; k_0__ < tau_beta_int_k_0_max__; ++k_0__) {
                if (stan::math::is_uninitialized(tau_beta_int[k_0__])) {
                    std::stringstream msg__;
                    msg__ << "Undefined transformed parameter: tau_beta_int" << "[" << k_0__ << "]";
                    stan::lang::rethrow_located(std::runtime_error(std::string("Error initializing variable tau_beta_int: ") + msg__.str()), current_statement_begin__, prog_reader__());
                }
            }
            size_t tau_beta_int_i_0_max__ = q;
            for (size_t i_0__ = 0; i_0__ < tau_beta_int_i_0_max__; ++i_0__) {
                check_greater_or_equal(function__, "tau_beta_int[i_0__]", tau_beta_int[i_0__], 0);
            }
            // model body
            current_statement_begin__ = 158;
            lp_accum__.add(normal_log<propto__>(mu_beta0, 0, 100));
            current_statement_begin__ = 159;
            lp_accum__.add(normal_log<propto__>(beta0_p_tilde, 0, 1));
            current_statement_begin__ = 160;
            lp_accum__.add(normal_log<propto__>(beta0_t_tilde, 0, 1));
            current_statement_begin__ = 161;
            lp_accum__.add(cauchy_log<propto__>(s_beta0_p, 0, 2.5));
            current_statement_begin__ = 162;
            lp_accum__.add(cauchy_log<propto__>(s_beta0_t, 0, 2.5));
            current_statement_begin__ = 165;
            lp_accum__.add(normal_log<propto__>(beta, 0, 100));
            current_statement_begin__ = 168;
            lp_accum__.add(cauchy_log<propto__>(sigma_beta, 0, 2.5));
            current_statement_begin__ = 169;
            for (int i = 1; i <= p; ++i) {
                current_statement_begin__ = 170;
                lp_accum__.add(normal_log<propto__>(get_base1(get_base1(beta_bs, i, "beta_bs", 1), 1, "beta_bs", 2), 0, 1));
                current_statement_begin__ = 171;
                for (int k = 2; k <= df; ++k) {
                    current_statement_begin__ = 172;
                    lp_accum__.add(normal_log<propto__>(get_base1(get_base1(beta_bs, i, "beta_bs", 1), k, "beta_bs", 2), get_base1(get_base1(beta_bs, i, "beta_bs", 1), (k - 1), "beta_bs", 2), get_base1(sigma_beta, i, "sigma_beta", 1)));
                }
            }
            current_statement_begin__ = 177;
            lp_accum__.add(cauchy_log<propto__>(sigma_beta_int, 0, 2.5));
            current_statement_begin__ = 178;
            for (int j = 1; j <= q; ++j) {
                current_statement_begin__ = 179;
                lp_accum__.add(sparse_iar_lpdf<propto__>(get_base1(beta_int, j, "beta_int", 1), get_base1(tau_beta_int, j, "tau_beta_int", 1), W_sparse, D_sparse, lambda, df_int, W_n, pstream__));
            }
            current_statement_begin__ = 183;
            lp_accum__.add(cauchy_log<propto__>(sigma_y, 0, 2.5));
            current_statement_begin__ = 186;
            lp_accum__.add(normal_log<propto__>(y, mu, sigma_y));
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
            throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
        }
        lp_accum__.add(lp__);
        return lp_accum__.sum();
    } // log_prob()
    template <bool propto, bool jacobian, typename T_>
    T_ log_prob(Eigen::Matrix<T_,Eigen::Dynamic,1>& params_r,
               std::ostream* pstream = 0) const {
      std::vector<T_> vec_params_r;
      vec_params_r.reserve(params_r.size());
      for (int i = 0; i < params_r.size(); ++i)
        vec_params_r.push_back(params_r(i));
      std::vector<int> vec_params_i;
      return log_prob<propto,jacobian,T_>(vec_params_r, vec_params_i, pstream);
    }
    void get_param_names(std::vector<std::string>& names__) const {
        names__.resize(0);
        names__.push_back("mu_beta0");
        names__.push_back("beta0_p_tilde");
        names__.push_back("s_beta0_p");
        names__.push_back("beta0_t_tilde");
        names__.push_back("s_beta0_t");
        names__.push_back("beta");
        names__.push_back("beta_bs");
        names__.push_back("sigma_beta");
        names__.push_back("beta_int");
        names__.push_back("sigma_beta_int");
        names__.push_back("sigma_y");
        names__.push_back("beta0_p");
        names__.push_back("beta0_t");
        names__.push_back("mu");
        names__.push_back("tau_beta_int");
        names__.push_back("y_rep");
        names__.push_back("y_pred");
        names__.push_back("log_lik");
    }
    void get_dims(std::vector<std::vector<size_t> >& dimss__) const {
        dimss__.resize(0);
        std::vector<size_t> dims__;
        dims__.resize(0);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n_plot);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n_tree);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(K);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(p);
        dims__.push_back(df);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(p);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(q);
        dims__.push_back(df_int);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(q);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n_plot);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n_tree);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(q);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n_pred);
        dimss__.push_back(dims__);
        dims__.resize(0);
        dims__.push_back(n);
        dimss__.push_back(dims__);
    }
    template <typename RNG>
    void write_array(RNG& base_rng__,
                     std::vector<double>& params_r__,
                     std::vector<int>& params_i__,
                     std::vector<double>& vars__,
                     bool include_tparams__ = true,
                     bool include_gqs__ = true,
                     std::ostream* pstream__ = 0) const {
        typedef double local_scalar_t__;
        vars__.resize(0);
        stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);
        static const char* function__ = "model_splines_interaction_sparse_namespace::write_array";
        (void) function__;  // dummy to suppress unused var warning
        // read-transform, write parameters
        double mu_beta0 = in__.scalar_constrain();
        vars__.push_back(mu_beta0);
        std::vector<double> beta0_p_tilde;
        size_t beta0_p_tilde_d_0_max__ = n_plot;
        beta0_p_tilde.reserve(beta0_p_tilde_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < beta0_p_tilde_d_0_max__; ++d_0__) {
            beta0_p_tilde.push_back(in__.scalar_constrain());
        }
        size_t beta0_p_tilde_k_0_max__ = n_plot;
        for (size_t k_0__ = 0; k_0__ < beta0_p_tilde_k_0_max__; ++k_0__) {
            vars__.push_back(beta0_p_tilde[k_0__]);
        }
        double s_beta0_p = in__.scalar_lb_constrain(0);
        vars__.push_back(s_beta0_p);
        std::vector<double> beta0_t_tilde;
        size_t beta0_t_tilde_d_0_max__ = n_tree;
        beta0_t_tilde.reserve(beta0_t_tilde_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < beta0_t_tilde_d_0_max__; ++d_0__) {
            beta0_t_tilde.push_back(in__.scalar_constrain());
        }
        size_t beta0_t_tilde_k_0_max__ = n_tree;
        for (size_t k_0__ = 0; k_0__ < beta0_t_tilde_k_0_max__; ++k_0__) {
            vars__.push_back(beta0_t_tilde[k_0__]);
        }
        double s_beta0_t = in__.scalar_lb_constrain(0);
        vars__.push_back(s_beta0_t);
        Eigen::Matrix<double, Eigen::Dynamic, 1> beta = in__.vector_constrain(K);
        size_t beta_j_1_max__ = K;
        for (size_t j_1__ = 0; j_1__ < beta_j_1_max__; ++j_1__) {
            vars__.push_back(beta(j_1__));
        }
        std::vector<Eigen::Matrix<double, Eigen::Dynamic, 1> > beta_bs;
        size_t beta_bs_d_0_max__ = p;
        beta_bs.reserve(beta_bs_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < beta_bs_d_0_max__; ++d_0__) {
            beta_bs.push_back(in__.vector_constrain(df));
        }
        size_t beta_bs_j_1_max__ = df;
        size_t beta_bs_k_0_max__ = p;
        for (size_t j_1__ = 0; j_1__ < beta_bs_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_bs_k_0_max__; ++k_0__) {
                vars__.push_back(beta_bs[k_0__](j_1__));
            }
        }
        std::vector<double> sigma_beta;
        size_t sigma_beta_d_0_max__ = p;
        sigma_beta.reserve(sigma_beta_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < sigma_beta_d_0_max__; ++d_0__) {
            sigma_beta.push_back(in__.scalar_lb_constrain(0));
        }
        size_t sigma_beta_k_0_max__ = p;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_k_0_max__; ++k_0__) {
            vars__.push_back(sigma_beta[k_0__]);
        }
        std::vector<Eigen::Matrix<double, Eigen::Dynamic, 1> > beta_int;
        size_t beta_int_d_0_max__ = q;
        beta_int.reserve(beta_int_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < beta_int_d_0_max__; ++d_0__) {
            beta_int.push_back(in__.vector_constrain(df_int));
        }
        size_t beta_int_j_1_max__ = df_int;
        size_t beta_int_k_0_max__ = q;
        for (size_t j_1__ = 0; j_1__ < beta_int_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_int_k_0_max__; ++k_0__) {
                vars__.push_back(beta_int[k_0__](j_1__));
            }
        }
        std::vector<double> sigma_beta_int;
        size_t sigma_beta_int_d_0_max__ = q;
        sigma_beta_int.reserve(sigma_beta_int_d_0_max__);
        for (size_t d_0__ = 0; d_0__ < sigma_beta_int_d_0_max__; ++d_0__) {
            sigma_beta_int.push_back(in__.scalar_lb_constrain(0));
        }
        size_t sigma_beta_int_k_0_max__ = q;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_int_k_0_max__; ++k_0__) {
            vars__.push_back(sigma_beta_int[k_0__]);
        }
        double sigma_y = in__.scalar_lb_constrain(0);
        vars__.push_back(sigma_y);
        double lp__ = 0.0;
        (void) lp__;  // dummy to suppress unused var warning
        stan::math::accumulator<double> lp_accum__;
        local_scalar_t__ DUMMY_VAR__(std::numeric_limits<double>::quiet_NaN());
        (void) DUMMY_VAR__;  // suppress unused var warning
        if (!include_tparams__ && !include_gqs__) return;
        try {
            // declare and define transformed parameters
            current_statement_begin__ = 121;
            validate_non_negative_index("beta0_p", "n_plot", n_plot);
            std::vector<double> beta0_p(n_plot, double(0));
            stan::math::initialize(beta0_p, DUMMY_VAR__);
            stan::math::fill(beta0_p, DUMMY_VAR__);
            current_statement_begin__ = 122;
            validate_non_negative_index("beta0_t", "n_tree", n_tree);
            std::vector<double> beta0_t(n_tree, double(0));
            stan::math::initialize(beta0_t, DUMMY_VAR__);
            stan::math::fill(beta0_t, DUMMY_VAR__);
            current_statement_begin__ = 123;
            validate_non_negative_index("mu", "n", n);
            Eigen::Matrix<double, Eigen::Dynamic, 1> mu(n);
            stan::math::initialize(mu, DUMMY_VAR__);
            stan::math::fill(mu, DUMMY_VAR__);
            current_statement_begin__ = 124;
            validate_non_negative_index("tau_beta_int", "q", q);
            std::vector<double> tau_beta_int(q, double(0));
            stan::math::initialize(tau_beta_int, DUMMY_VAR__);
            stan::math::fill(tau_beta_int, DUMMY_VAR__);
            // do transformed parameters statements
            current_statement_begin__ = 126;
            for (int j = 1; j <= n_plot; ++j) {
                current_statement_begin__ = 127;
                stan::model::assign(beta0_p, 
                            stan::model::cons_list(stan::model::index_uni(j), stan::model::nil_index_list()), 
                            (mu_beta0 + (s_beta0_p * get_base1(beta0_p_tilde, j, "beta0_p_tilde", 1))), 
                            "assigning variable beta0_p");
            }
            current_statement_begin__ = 130;
            for (int t = 1; t <= n_tree; ++t) {
                current_statement_begin__ = 131;
                stan::model::assign(beta0_t, 
                            stan::model::cons_list(stan::model::index_uni(t), stan::model::nil_index_list()), 
                            (get_base1(beta0_p, get_base1(plot_by_tree_idx, t, "plot_by_tree_idx", 1), "beta0_p", 1) + (s_beta0_t * get_base1(beta0_t_tilde, t, "beta0_t_tilde", 1))), 
                            "assigning variable beta0_t");
            }
            current_statement_begin__ = 134;
            for (int i = 1; i <= n; ++i) {
                {
                current_statement_begin__ = 136;
                local_scalar_t__ tmp(DUMMY_VAR__);
                (void) tmp;  // dummy to suppress unused var warning
                stan::math::initialize(tmp, DUMMY_VAR__);
                stan::math::fill(tmp, DUMMY_VAR__);
                stan::math::assign(tmp,0);
                current_statement_begin__ = 137;
                local_scalar_t__ tmp_int(DUMMY_VAR__);
                (void) tmp_int;  // dummy to suppress unused var warning
                stan::math::initialize(tmp_int, DUMMY_VAR__);
                stan::math::fill(tmp_int, DUMMY_VAR__);
                stan::math::assign(tmp_int,0);
                current_statement_begin__ = 140;
                for (int j = 1; j <= p; ++j) {
                    current_statement_begin__ = 141;
                    stan::math::assign(tmp, (tmp + multiply(get_base1(get_base1(X_bs, j, "X_bs", 1), i, "X_bs", 2), get_base1(beta_bs, j, "beta_bs", 1))));
                }
                current_statement_begin__ = 144;
                for (int j = 1; j <= q; ++j) {
                    current_statement_begin__ = 145;
                    stan::math::assign(tmp_int, (tmp_int + multiply(get_base1(get_base1(X_bs_int, j, "X_bs_int", 1), i, "X_bs_int", 2), get_base1(beta_int, j, "beta_int", 1))));
                }
                current_statement_begin__ = 147;
                stan::model::assign(mu, 
                            stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                            (((get_base1(beta0_t, get_base1(tree_idx, i, "tree_idx", 1), "beta0_t", 1) + multiply(get_base1(X, i, "X", 1), beta)) + tmp) + tmp_int), 
                            "assigning variable mu");
                }
            }
            current_statement_begin__ = 150;
            for (int j = 1; j <= q; ++j) {
                current_statement_begin__ = 151;
                stan::model::assign(tau_beta_int, 
                            stan::model::cons_list(stan::model::index_uni(j), stan::model::nil_index_list()), 
                            (1 / get_base1(sigma_beta_int, j, "sigma_beta_int", 1)), 
                            "assigning variable tau_beta_int");
            }
            if (!include_gqs__ && !include_tparams__) return;
            // validate transformed parameters
            const char* function__ = "validate transformed params";
            (void) function__;  // dummy to suppress unused var warning
            current_statement_begin__ = 124;
            size_t tau_beta_int_i_0_max__ = q;
            for (size_t i_0__ = 0; i_0__ < tau_beta_int_i_0_max__; ++i_0__) {
                check_greater_or_equal(function__, "tau_beta_int[i_0__]", tau_beta_int[i_0__], 0);
            }
            // write transformed parameters
            if (include_tparams__) {
                size_t beta0_p_k_0_max__ = n_plot;
                for (size_t k_0__ = 0; k_0__ < beta0_p_k_0_max__; ++k_0__) {
                    vars__.push_back(beta0_p[k_0__]);
                }
                size_t beta0_t_k_0_max__ = n_tree;
                for (size_t k_0__ = 0; k_0__ < beta0_t_k_0_max__; ++k_0__) {
                    vars__.push_back(beta0_t[k_0__]);
                }
                size_t mu_j_1_max__ = n;
                for (size_t j_1__ = 0; j_1__ < mu_j_1_max__; ++j_1__) {
                    vars__.push_back(mu(j_1__));
                }
                size_t tau_beta_int_k_0_max__ = q;
                for (size_t k_0__ = 0; k_0__ < tau_beta_int_k_0_max__; ++k_0__) {
                    vars__.push_back(tau_beta_int[k_0__]);
                }
            }
            if (!include_gqs__) return;
            // declare and define generated quantities
            current_statement_begin__ = 191;
            validate_non_negative_index("y_rep", "n", n);
            Eigen::Matrix<double, Eigen::Dynamic, 1> y_rep(n);
            stan::math::initialize(y_rep, DUMMY_VAR__);
            stan::math::fill(y_rep, DUMMY_VAR__);
            current_statement_begin__ = 192;
            validate_non_negative_index("y_pred", "n_pred", n_pred);
            Eigen::Matrix<double, Eigen::Dynamic, 1> y_pred(n_pred);
            stan::math::initialize(y_pred, DUMMY_VAR__);
            stan::math::fill(y_pred, DUMMY_VAR__);
            current_statement_begin__ = 193;
            validate_non_negative_index("log_lik", "n", n);
            Eigen::Matrix<double, Eigen::Dynamic, 1> log_lik(n);
            stan::math::initialize(log_lik, DUMMY_VAR__);
            stan::math::fill(log_lik, DUMMY_VAR__);
            // generated quantities statements
            current_statement_begin__ = 195;
            for (int i = 1; i <= n; ++i) {
                {
                current_statement_begin__ = 197;
                local_scalar_t__ tmp(DUMMY_VAR__);
                (void) tmp;  // dummy to suppress unused var warning
                stan::math::initialize(tmp, DUMMY_VAR__);
                stan::math::fill(tmp, DUMMY_VAR__);
                stan::math::assign(tmp,0);
                current_statement_begin__ = 198;
                local_scalar_t__ tmp_int(DUMMY_VAR__);
                (void) tmp_int;  // dummy to suppress unused var warning
                stan::math::initialize(tmp_int, DUMMY_VAR__);
                stan::math::fill(tmp_int, DUMMY_VAR__);
                stan::math::assign(tmp_int,0);
                current_statement_begin__ = 199;
                for (int j = 1; j <= p; ++j) {
                    current_statement_begin__ = 200;
                    stan::math::assign(tmp, (tmp + multiply(get_base1(get_base1(X_bs, j, "X_bs", 1), i, "X_bs", 2), get_base1(beta_bs, j, "beta_bs", 1))));
                }
                current_statement_begin__ = 202;
                for (int j = 1; j <= q; ++j) {
                    current_statement_begin__ = 203;
                    stan::math::assign(tmp_int, (tmp_int + multiply(get_base1(get_base1(X_bs_int, j, "X_bs_int", 1), i, "X_bs_int", 2), get_base1(beta_int, j, "beta_int", 1))));
                }
                current_statement_begin__ = 206;
                stan::model::assign(y_rep, 
                            stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                            normal_rng(((get_base1(beta0_t, get_base1(tree_idx, i, "tree_idx", 1), "beta0_t", 1) + multiply(get_base1(X, i, "X", 1), beta)) + tmp), sigma_y, base_rng__), 
                            "assigning variable y_rep");
                current_statement_begin__ = 207;
                stan::model::assign(log_lik, 
                            stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                            normal_log(get_base1(y, i, "y", 1), ((get_base1(beta0_t, get_base1(tree_idx, i, "tree_idx", 1), "beta0_t", 1) + multiply(get_base1(X, i, "X", 1), beta)) + tmp), sigma_y), 
                            "assigning variable log_lik");
                }
            }
            current_statement_begin__ = 209;
            for (int i = 1; i <= n_pred; ++i) {
                {
                current_statement_begin__ = 211;
                local_scalar_t__ tmp(DUMMY_VAR__);
                (void) tmp;  // dummy to suppress unused var warning
                stan::math::initialize(tmp, DUMMY_VAR__);
                stan::math::fill(tmp, DUMMY_VAR__);
                stan::math::assign(tmp,0);
                current_statement_begin__ = 212;
                local_scalar_t__ tmp_int(DUMMY_VAR__);
                (void) tmp_int;  // dummy to suppress unused var warning
                stan::math::initialize(tmp_int, DUMMY_VAR__);
                stan::math::fill(tmp_int, DUMMY_VAR__);
                stan::math::assign(tmp_int,0);
                current_statement_begin__ = 213;
                for (int j = 1; j <= p; ++j) {
                    current_statement_begin__ = 214;
                    stan::math::assign(tmp, (tmp + multiply(get_base1(get_base1(X_bs_pred, j, "X_bs_pred", 1), i, "X_bs_pred", 2), get_base1(beta_bs, j, "beta_bs", 1))));
                }
                current_statement_begin__ = 216;
                for (int j = 1; j <= q; ++j) {
                    current_statement_begin__ = 217;
                    stan::math::assign(tmp_int, (tmp_int + multiply(get_base1(get_base1(X_bs_int_pred, j, "X_bs_int_pred", 1), i, "X_bs_int_pred", 2), get_base1(beta_int, j, "beta_int", 1))));
                }
                current_statement_begin__ = 219;
                stan::model::assign(y_pred, 
                            stan::model::cons_list(stan::model::index_uni(i), stan::model::nil_index_list()), 
                            normal_rng(((get_base1(beta0_t, get_base1(tree_idx_pred, i, "tree_idx_pred", 1), "beta0_t", 1) + multiply(get_base1(X_pred, i, "X_pred", 1), beta)) + tmp), sigma_y, base_rng__), 
                            "assigning variable y_pred");
                }
            }
            // validate, write generated quantities
            current_statement_begin__ = 191;
            size_t y_rep_j_1_max__ = n;
            for (size_t j_1__ = 0; j_1__ < y_rep_j_1_max__; ++j_1__) {
                vars__.push_back(y_rep(j_1__));
            }
            current_statement_begin__ = 192;
            size_t y_pred_j_1_max__ = n_pred;
            for (size_t j_1__ = 0; j_1__ < y_pred_j_1_max__; ++j_1__) {
                vars__.push_back(y_pred(j_1__));
            }
            current_statement_begin__ = 193;
            size_t log_lik_j_1_max__ = n;
            for (size_t j_1__ = 0; j_1__ < log_lik_j_1_max__; ++j_1__) {
                vars__.push_back(log_lik(j_1__));
            }
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e, current_statement_begin__, prog_reader__());
            // Next line prevents compiler griping about no return
            throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
        }
    }
    template <typename RNG>
    void write_array(RNG& base_rng,
                     Eigen::Matrix<double,Eigen::Dynamic,1>& params_r,
                     Eigen::Matrix<double,Eigen::Dynamic,1>& vars,
                     bool include_tparams = true,
                     bool include_gqs = true,
                     std::ostream* pstream = 0) const {
      std::vector<double> params_r_vec(params_r.size());
      for (int i = 0; i < params_r.size(); ++i)
        params_r_vec[i] = params_r(i);
      std::vector<double> vars_vec;
      std::vector<int> params_i_vec;
      write_array(base_rng, params_r_vec, params_i_vec, vars_vec, include_tparams, include_gqs, pstream);
      vars.resize(vars_vec.size());
      for (int i = 0; i < vars.size(); ++i)
        vars(i) = vars_vec[i];
    }
    std::string model_name() const {
        return "model_splines_interaction_sparse";
    }
    void constrained_param_names(std::vector<std::string>& param_names__,
                                 bool include_tparams__ = true,
                                 bool include_gqs__ = true) const {
        std::stringstream param_name_stream__;
        param_name_stream__.str(std::string());
        param_name_stream__ << "mu_beta0";
        param_names__.push_back(param_name_stream__.str());
        size_t beta0_p_tilde_k_0_max__ = n_plot;
        for (size_t k_0__ = 0; k_0__ < beta0_p_tilde_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "beta0_p_tilde" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
        param_name_stream__ << "s_beta0_p";
        param_names__.push_back(param_name_stream__.str());
        size_t beta0_t_tilde_k_0_max__ = n_tree;
        for (size_t k_0__ = 0; k_0__ < beta0_t_tilde_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "beta0_t_tilde" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
        param_name_stream__ << "s_beta0_t";
        param_names__.push_back(param_name_stream__.str());
        size_t beta_j_1_max__ = K;
        for (size_t j_1__ = 0; j_1__ < beta_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "beta" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t beta_bs_j_1_max__ = df;
        size_t beta_bs_k_0_max__ = p;
        for (size_t j_1__ = 0; j_1__ < beta_bs_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_bs_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta_bs" << '.' << k_0__ + 1 << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        size_t sigma_beta_k_0_max__ = p;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "sigma_beta" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t beta_int_j_1_max__ = df_int;
        size_t beta_int_k_0_max__ = q;
        for (size_t j_1__ = 0; j_1__ < beta_int_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_int_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta_int" << '.' << k_0__ + 1 << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        size_t sigma_beta_int_k_0_max__ = q;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_int_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "sigma_beta_int" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
        param_name_stream__ << "sigma_y";
        param_names__.push_back(param_name_stream__.str());
        if (!include_gqs__ && !include_tparams__) return;
        if (include_tparams__) {
            size_t beta0_p_k_0_max__ = n_plot;
            for (size_t k_0__ = 0; k_0__ < beta0_p_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta0_p" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t beta0_t_k_0_max__ = n_tree;
            for (size_t k_0__ = 0; k_0__ < beta0_t_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta0_t" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t mu_j_1_max__ = n;
            for (size_t j_1__ = 0; j_1__ < mu_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "mu" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t tau_beta_int_k_0_max__ = q;
            for (size_t k_0__ = 0; k_0__ < tau_beta_int_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "tau_beta_int" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        if (!include_gqs__) return;
        size_t y_rep_j_1_max__ = n;
        for (size_t j_1__ = 0; j_1__ < y_rep_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "y_rep" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t y_pred_j_1_max__ = n_pred;
        for (size_t j_1__ = 0; j_1__ < y_pred_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "y_pred" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t log_lik_j_1_max__ = n;
        for (size_t j_1__ = 0; j_1__ < log_lik_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "log_lik" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
    }
    void unconstrained_param_names(std::vector<std::string>& param_names__,
                                   bool include_tparams__ = true,
                                   bool include_gqs__ = true) const {
        std::stringstream param_name_stream__;
        param_name_stream__.str(std::string());
        param_name_stream__ << "mu_beta0";
        param_names__.push_back(param_name_stream__.str());
        size_t beta0_p_tilde_k_0_max__ = n_plot;
        for (size_t k_0__ = 0; k_0__ < beta0_p_tilde_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "beta0_p_tilde" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
        param_name_stream__ << "s_beta0_p";
        param_names__.push_back(param_name_stream__.str());
        size_t beta0_t_tilde_k_0_max__ = n_tree;
        for (size_t k_0__ = 0; k_0__ < beta0_t_tilde_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "beta0_t_tilde" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
        param_name_stream__ << "s_beta0_t";
        param_names__.push_back(param_name_stream__.str());
        size_t beta_j_1_max__ = K;
        for (size_t j_1__ = 0; j_1__ < beta_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "beta" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t beta_bs_j_1_max__ = df;
        size_t beta_bs_k_0_max__ = p;
        for (size_t j_1__ = 0; j_1__ < beta_bs_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_bs_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta_bs" << '.' << k_0__ + 1 << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        size_t sigma_beta_k_0_max__ = p;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "sigma_beta" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t beta_int_j_1_max__ = df_int;
        size_t beta_int_k_0_max__ = q;
        for (size_t j_1__ = 0; j_1__ < beta_int_j_1_max__; ++j_1__) {
            for (size_t k_0__ = 0; k_0__ < beta_int_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta_int" << '.' << k_0__ + 1 << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        size_t sigma_beta_int_k_0_max__ = q;
        for (size_t k_0__ = 0; k_0__ < sigma_beta_int_k_0_max__; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "sigma_beta_int" << '.' << k_0__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
        param_name_stream__ << "sigma_y";
        param_names__.push_back(param_name_stream__.str());
        if (!include_gqs__ && !include_tparams__) return;
        if (include_tparams__) {
            size_t beta0_p_k_0_max__ = n_plot;
            for (size_t k_0__ = 0; k_0__ < beta0_p_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta0_p" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t beta0_t_k_0_max__ = n_tree;
            for (size_t k_0__ = 0; k_0__ < beta0_t_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "beta0_t" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t mu_j_1_max__ = n;
            for (size_t j_1__ = 0; j_1__ < mu_j_1_max__; ++j_1__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "mu" << '.' << j_1__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
            size_t tau_beta_int_k_0_max__ = q;
            for (size_t k_0__ = 0; k_0__ < tau_beta_int_k_0_max__; ++k_0__) {
                param_name_stream__.str(std::string());
                param_name_stream__ << "tau_beta_int" << '.' << k_0__ + 1;
                param_names__.push_back(param_name_stream__.str());
            }
        }
        if (!include_gqs__) return;
        size_t y_rep_j_1_max__ = n;
        for (size_t j_1__ = 0; j_1__ < y_rep_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "y_rep" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t y_pred_j_1_max__ = n_pred;
        for (size_t j_1__ = 0; j_1__ < y_pred_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "y_pred" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
        size_t log_lik_j_1_max__ = n;
        for (size_t j_1__ = 0; j_1__ < log_lik_j_1_max__; ++j_1__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "log_lik" << '.' << j_1__ + 1;
            param_names__.push_back(param_name_stream__.str());
        }
    }
}; // model
}  // namespace
typedef model_splines_interaction_sparse_namespace::model_splines_interaction_sparse stan_model;
#ifndef USING_R
stan::model::model_base& new_model(
        stan::io::var_context& data_context,
        unsigned int seed,
        std::ostream* msg_stream) {
  stan_model* m = new stan_model(data_context, seed, msg_stream);
  return *m;
}
#endif
#endif
